...
class DDPGLearner(ReinforcementLearner):
    ...
    # def get_batch(self):
    #     """Prepare the batch data for training the actor and critic."""
    #     memory = zip(
    #         reversed(self.memory_sample),
    #         reversed(self.memory_action),
    #         reversed(self.memory_reward),
    ...
    #     x = np.zeros((len(self.memory_sample), self.num_steps, self.num_features))
    #     y_actor = np.zeros((len(self.memory_sample), self.agent.NUM_ACTIONS))
    #     y_critic = np.zeros((len(self.memory_sample), 1))
    #     next_value = 0

    #     for i, (sample, action, reward) in enumerate(memory):
    #         x[i] = sample
    #         y_actor[i, action] = reward + self.discount_factor * next_value
    #         y_critic[i] = reward + self.discount_factor * next_value
    #         next_value = y_critic[i]
